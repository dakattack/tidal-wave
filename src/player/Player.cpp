...
void Player::initAudio()
{
    ...
    auto pushFreshPresenceFrame = [this, discordRPC, lastPushTime]()
    {
        // Fetch your structured tracking variable
        const auto trackMap = this->currentTrackMap();
        if (trackMap.isEmpty())
            return;
        m_currentTrack = trackFromMap(trackMap);
        title = m_currentTrack.title.isEmpty() ? QString("Unknown Track") : m_currentTrack.title;
        artist = m_currentTrack.artists.isEmpty() ? QString("Unknown Artist") : m_currentTrack.artists.first().name;
        albumArtUrl = m_currentTrack.coverUrl(1080).isEmpty() ? QString() : m_currentTrack.coverUrl(1080);
        currentPosMs = m_player->position() > 0 ? m_player->position() : 0;
        durationMs = m_currentTrack.duration > 0 ? m_currentTrack.duration * 1000 : 0;
        discordRPC->updatePresence(title, artist, albumArtUrl, currentPosMs, durationMs);
        lastPushTime->restart();
    };
    connect(this, &Player::sourceChanged, this, [this, pushFreshPresenceFrame]()
            { pushFreshPresenceFrame(); });
    connect(this, &Player::currentTrackChanged, this, [this, pushFreshPresenceFrame]()
            { pushFreshPresenceFrame(); });
    connect(this, &Player::playingChanged, this, [this, discordRPC, pushFreshPresenceFrame](bool playing)
            {
        if (playing) {
    ...
    connect(this, &Player::positionChanged, this, [this, lastPushTime, pushFreshPresenceFrame]()
            {
...
void Player::setPlaybackSource(const QString &type, const QString &id, const QString &name)
{
    ...
    if (m_sourceType == type && m_sourceId == id && m_sourceName == name)
        return;
    m_sourceType = type;
    m_sourceId = id;
    m_sourceName = name;
...
#ifdef Q_OS_LINUX
    if (casting() && m_castSession)
        m_castSession->setVolume(m_pendingVolume);
#endif
    if (m_audioOut)
        m_audioOut->setVolume(m_pendingVolume);
...
void Player::setMuted(bool m)
{
    ...
    if (m_audioOut && !casting())
        m_audioOut->setMuted(m);
    emit mutedChanged(m);
}
...
{
    if (m_repeatMode == 2)
        return m_index; // repeat one
    if (m_shuffle)
    {
        int si = m_shuffleOrder.indexOf(m_index);
        if (si < m_shuffleOrder.count() - 1)
            return m_shuffleOrder[si + 1];
        if (m_repeatMode == 1)
            return m_shuffleOrder[0];
        return -1;
    }
...
{
    if (!m_player || index < 0 || index >= m_queue.count())
        return;
...
QString Player::audioQuality() const
{
    ...
    if (m_streamedQuality == QStringLiteral("LOW"))
        maxQuality = AudioQuality::Low96k;
    else if (m_streamedQuality == QStringLiteral("HIGH"))
        maxQuality = AudioQuality::Low320k;
    else if (m_streamedQuality == QStringLiteral("LOSSLESS"))
        maxQuality = AudioQuality::Lossless;
    else if (m_streamedQuality == QStringLiteral("HI_RES_LOSSLESS"))
        maxQuality = AudioQuality::HiResLossless;
...
QString Player::qualityLabel(const QString &code) const
{
    ...
    if (code == QStringLiteral("HI_RES_LOSSLESS"))
        return QStringLiteral("Max");
    if (code == QStringLiteral("LOSSLESS"))
        return QStringLiteral("Lossless");
    if (code == QStringLiteral("HIGH"))
        return QStringLiteral("High");
    if (code == QStringLiteral("LOW"))
        return QStringLiteral("Low");
    return code;
}
...
void Player::onCastPosition(double sec)
{
    ...
    if (casting())
        emit positionChanged(m_castPosition);
}
...
void Player::onCastDuration(double sec)
{
    ...
    m_castDuration = d;
    if (casting())
        emit durationChanged(m_castDuration);
}

void Player::onCastPlaying(bool playing)
{
    if (m_castPlaying == playing)
        return;
